...
void jogo(JOGO copia_controle)  //ok
{
    ...
    while(condition != SAIR && condition != VITORIA && condition != DERROTA)
    {
        ...
        if (flag_gravidade==TRUE)  //se podae msm atuar a gravidade (frase estranha);
        {
            ...
            testa_queda((copia_controle.espelho_fixo),&coord_x,&coord_y,&eh_valido,&reposicao);
...
int move_mario(JOGO *parametros, char key)
{
    ...
    int eh_valido=0;
    int pulando;
    if(parametros->espelho_fixo.controle[parametros->mario.coluna_inicial][parametros->mario.linha_inicial+2] == ' ')
        pulando=1;
    else
        pulando=0;
    ...
    case  'P':
        return PAUSE;
    case SPACEBAR: if(pulando==0)
                   {
                       apaga(parametros->mario.coluna_inicial,parametros->mario.linha_inicial);
                       parametros->mario.linha_inicial-=2;
                       pulando=TRUE;
                       posiciona_mario(parametros->mario.coluna_inicial,parametros->mario.linha_inicial);
                   }
        break;
    }
